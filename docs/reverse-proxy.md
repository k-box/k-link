---
Title: Reverse proxy
Description: How to deploy the K-Link behind a proxy
---

The K-Link can be exposed behind a reverse proxy. It is actually suggested to do so, as the K-Link configuration do not natively support secure connections. 
In addition, you might have more services on your domain and on your server, therefore you might want to expose the K-Link on a path on on a sub-domain using the same server that is managing a website.
...
Considering the default [K-Link installation](./installation.md), to use Traefik as your reverse proxy, a few configurations are necessary.

#### Configuring an starting Traefik

As the first step Traefik needs to be installed on the server. For simplification purposes we will install Traefik on the same machine as the K-Link.

Let's create a folder, called `reverseproxy`, inside the `~/deploy/` folder used for the K-Link.
...
#### Update the K-Link installation configuration to use Traefik

Once the Traefik service is up and running, we can apply the required change to the K-Link docker-compose.yml configuration.
...
+  labels:
+    - "traefik.enable=true"
+    - "traefik.frontend.rule=Host: my.klink.tld"
+    - "traefik.docker.network=reverseproxy_web"
```

The `traefik.frontend.rule=Host: my.klink.tld` tells Traefik that the service will be reachable if the user issue a request to my.klink.tld, using http or https, as defined by Traefik listening on port 80 and 443.
...
After changing the configuration you should restart the K-Link service so Traefik will be notified. At this point Traefik will take care of generating the SSL certificate.
